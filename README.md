...
|Алгоритм для "Горящих" билетов |Поиск и уведомление о резком снижении цены / "горящих" предложениях |1) Непрерывный мониторинг цен <br>Для популярных направлений и ближайших дат мы регулярно фиксируем "снимки" цен: 
<br>`price_now(route, date, airline)` <br> и храним короткую историю изменений за последние N дней или часов. <br>2) Детектор резкого снижения относительно нормы <br>По историческим данным строим ценовой эталон:
<br>`baseline_price(d) = median(price_hist \| route, season, dow, days_to_dep = d)` <br> Далее оцениваем отклонение: <br>`drop = (baseline_price(d_now) - price_now) / baseline_price(d_now)` <br> Рейс считается кандидатом в "горящие", если: <br>
- снижении цены превышает порог τ_drop (обычно 20–40%+); <br>
- до вылета остаётся меньше τ_days. <br>
3) Проверка доступности и устойчивости цены <br>
Чтобы исключить «фантомные» предложения, повторно перепроверяем оффер у поставщика, требуем минимальное количество доступных мест/тарифов и отбрасываем некорректные варианты (невозможные стыковки, скрытые доплаты и т. п.). <br>
4) Промо-сигналы и распродажи <br>
Учитываем данные от авиакомпаний и агентств, а также распознаваем характерные паттерны промо-тарифов.<br>
Если `promo_flag = detect_promo(airline_feed, price_pattern)`, <br>
то предложение может попасть в список «горящих» даже при большем горизонте до вылета. <br>
5) Финальный скоринг и ранжирование <br>
Итоговый показатель формируется из размера скидки, срочности и качества маршрута: <br>
`hot_score = w_drop * normalize(drop) + w_urg * urgency(days_to_departure) - w_q * quality_penalty + w_promo * promo_flag` <br>
Предложения сортируются по hot_score и выводятся в подборку.<br>
6) Короткий срок жизни офферов <br>
"Горящие" билеты живут недолго, поэтому каждая карточка имеет небольшой TTL. <br>
Мы регулярно переоцениваем цену и снимаем оффер, если он перестаёт быть выгодным. |
